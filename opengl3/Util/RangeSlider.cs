...
public class RangeSliderVinv : UserControl
{
    public int MinValue { get; set; } = 0;
    public int MaxValue { get; set; } = 100;
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        // Вычисляем Y-координаты ползунков (сверху – меньшее значение, снизу – большее)
        int lowerY = (int)((double)(LowerValue - MinValue) / (MaxValue - MinValue) * lineHeight);
        int upperY = (int)((double)(UpperValue - MinValue) / (MaxValue - MinValue) * lineHeight);

        // Фоновая линия
        g.FillRectangle(Brushes.LightGray, lineX, 0, 4, Height);

        // Активная область между центрами ползунков
        int centerLower = lowerY + thumbHeight / 2;
        int centerUpper = upperY + thumbHeight / 2;
        int activeTop = Math.Min(centerLower, centerUpper);
        int activeBottom = Math.Max(centerLower, centerUpper);
        g.FillRectangle(Brushes.DodgerBlue, lineX, activeTop, 4, activeBottom - activeTop);
    ...
    protected override void OnMouseMove(MouseEventArgs e)
    {
        ...
        if (draggingLower)
        {
            // Процент от 0 (сверху) до 1 (снизу)
            double percent = (double)(e.Y - thumbHeight / 2) / lineHeight;
