	...
	public class JKTYPE
	{
		...
		*/
		...
		public struct SignInfo{
			...
			public string sig_name;//标识名
			public int chn_id;		//RS485通道ID
			public int sig_type;	//信号量类型
			public int sig_addr;	//寄存器地址
			public int value;		//值  设置时无效
			public int frequency;	//信号量在控制器内部刷新频率不大于10
		}

		[StructLayout(LayoutKind.Sequential)]
		public struct ModRtuComm
		{
			public int chn_id;		//RS485通道ID  查询时chn_id作为输入参数
			public int slaveId;	//当通道模式设置为Modbus RTU时，需额外指定Modbus从站节点ID，其余模式可忽略
			public int baudrate;	//波特率4800,9600,14400,19200,38400,57600,115200,230400
			public int databit;	//数据位7，8
			public int stopbit;	//停止位1，2
			public int parity;		//校验位78-> 无校验 79->奇校验 69->偶校验
		}
	}
}
