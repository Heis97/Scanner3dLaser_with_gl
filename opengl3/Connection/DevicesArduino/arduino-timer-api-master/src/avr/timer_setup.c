...
void timer_init_ISR(int timer, int prescaler, unsigned int adjustment) {
    ...
    if(timer == _timer2) {
        ...
        TCCR2B = 0;             // timer mode:
        TCCR2B |= _BV(WGM22);   // CTC mode
        TCCR2B |= prescalerBits;// set prescaler
        OCR2A = adjustment;     // compare match register
...
#endif
#if defined(WIRING)
        timerAttach(TIMER2OUTCOMPAREA_INT, Timer2Service);
#endif
    }
#endif
...
#else
        // here if not ATmega8 or ATmega128
        TIMSK2 &= ~_BV(OCIE2A); // disable output compare interrupt
#endif
    }
#endif
