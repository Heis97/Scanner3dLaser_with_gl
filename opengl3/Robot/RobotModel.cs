...
namespace opengl3
{
    class RobotModel
    {
        ...
        const int _t_per = 100;
        const double _msToSec = 0.001;
        public RobotModel(robFrame rF, int port)
        {
            _TCPserver = new TCPserver(port);
            Thread server_thread = new Thread(_TCPserver.startServer);
            server_thread.Start();
            _robFrame = rF;
            _RobLine = new RobLine(this, _msToSec);
           
            TimerCallback tm = new TimerCallback(compPosic);
            Timer timer = new Timer(tm, 0, 0, _t_per);
        }
