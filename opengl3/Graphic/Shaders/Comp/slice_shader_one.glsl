...
void main() 
{
	...
	//imageStore(isolines, ivec2(gl_GlobalInvocationID.x,0), vec4(gl_GlobalInvocationID.x,2, 4, 8));
	...
	for (int i=0; i<3;i++)
	{
		...
		imageStore(isolines, ivec2(3*gl_GlobalInvocationID.x+i,1), vec4(vertexPosition_world[i], 1));	
	}
		
	find_points_isoline(vec4(0,0,1,-2),vertexPosition_world);
